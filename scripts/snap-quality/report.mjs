...
export function summarize(results) {
  ...
  return {
    ...
    meanRgbSampleCountTotal: results.reduce((sum, item) => sum + (item.meanRgbSampleCount ?? 0), 0),
    meanRgbRDriftMean: mean(results, 'meanRgbRDrift'),
    meanRgbGDriftMean: mean(results, 'meanRgbGDrift'),
    meanRgbBDriftMean: mean(results, 'meanRgbBDrift'),
    ...
    sourceInkPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.sourceInkPixelCount ?? 0),
      0,
    ),
    outputInkPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.outputInkPixelCount ?? 0),
      0,
    ),
    inputInkCoverageMean: mean(results, 'inputInkCoverage'),
    outputInkCoverageMean: mean(results, 'outputInkCoverage'),
    inkCoverageDriftMean: mean(results, 'inkCoverageDrift'),
    inkCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.inkCoverageDrift ?? 0),
      0,
    ),
    sourceInkComponentPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.sourceInkComponentPixelCount ?? 0),
      0,
    ),
    outputInkComponentPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.outputInkComponentPixelCount ?? 0),
      0,
    ),
    inkComponentCoverageDriftMean: mean(results, 'inkComponentCoverageDrift'),
    inkComponentCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.inkComponentCoverageDrift ?? 0),
      0,
    ),
    inkLargestComponentCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.inkLargestComponentCoverageDrift ?? 0),
      0,
    ),
    sourceShadowPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.sourceShadowPixelCount ?? 0),
      0,
    ),
    outputShadowPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.outputShadowPixelCount ?? 0),
      0,
    ),
    inputShadowCoverageMean: mean(results, 'inputShadowCoverage'),
    outputShadowCoverageMean: mean(results, 'outputShadowCoverage'),
    shadowCoverageDriftMean: mean(results, 'shadowCoverageDrift'),
    shadowCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.shadowCoverageDrift ?? 0),
      0,
    ),
    tileShadowCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.tileShadowCoverageDriftMax ?? 0),
      0,
    ),
    tileShadowCoverageDriftP95Max: results.reduce(
      (max, item) => Math.max(max, item.tileShadowCoverageDriftP95 ?? 0),
      0,
    ),
    tileShadowCoverageTileCountTotal: results.reduce(
      (sum, item) => sum + (item.tileShadowCoverageTileCount ?? 0),
      0,
    ),
    ...
    outputBrightPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.outputBrightPixelCount ?? 0),
      0,
    ),
    inputBrightCoverageMean: mean(results, 'inputBrightCoverage'),
    outputBrightCoverageMean: mean(results, 'outputBrightCoverage'),
    brightCoverageDriftMean: mean(results, 'brightCoverageDrift'),
    brightCoverageDriftMax: results.reduce(
    ...
    outputSaturatedPixelCountTotal: results.reduce(
      (sum, item) => sum + (item.outputSaturatedPixelCount ?? 0),
      0,
    ),
    inputSaturatedCoverageMean: mean(results, 'inputSaturatedCoverage'),
    outputSaturatedCoverageMean: mean(results, 'outputSaturatedCoverage'),
    saturatedCoverageDriftMean: mean(results, 'saturatedCoverageDrift'),
    saturatedCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.saturatedCoverageDrift ?? 0),
      0,
    ),
    tileSaturatedCoverageDriftMax: results.reduce(
      (max, item) => Math.max(max, item.tileSaturatedCoverageDriftMax ?? 0),
      0,
    ),
    tileSaturatedCoverageDriftP95Max: results.reduce(
      (max, item) => Math.max(max, item.tileSaturatedCoverageDriftP95 ?? 0),
      0,
    ),
    tileSaturatedCoverageTileCountTotal: results.reduce(
      (sum, item) => sum + (item.tileSaturatedCoverageTileCount ?? 0),
      0,
    ),
...
function criteriaMarkdown() {
  return [
    ...
    '- Preserve-detail expectations: generated pseudo pixel-art samples must match the accepted detail-preserving grid; true-grid-only checks such as micro-grid, repeat stability, and source-cell-size are not used as pass/fail signals for that mode.',
    ...
    `- Ink component coverage: review when connected ink components of at least the larger of ${QUALITY_RULES.minInkComponentArea} pixels and ${(QUALITY_RULES.minInkComponentAreaRatio * 100).toFixed(3)}% of image area cover at least ${QUALITY_RULES.minInkComponentPixelCount} source or output pixels and drift by more than ${(QUALITY_RULES.maxInkComponentCoverageDrift * 100).toFixed(1)} percentage points.`,
    ...
    `- Regional shadow coverage: review when eligible 8x8 canvas tile p95 changes coverage of pixels below luma ${QUALITY_RULES.shadowCoverageLumaThreshold} by more than ${(QUALITY_RULES.maxTileShadowCoverageDriftP95 * 100).toFixed(0)} percentage points.`,
    ...
    `- Regional saturated color coverage: review when eligible 8x8 canvas tile p95 changes coverage of pixels above chroma ${QUALITY_RULES.saturatedCoverageChromaThreshold} by more than ${(QUALITY_RULES.maxTileSaturatedCoverageDriftP95 * 100).toFixed(0)} percentage points.`,
...
export function toMarkdown(summary) {
  const headers = [
    ...
    'inkCoverageDrift',
    'inkComponentMinArea',
    'sourceInkComponentCount',
    'outputInkComponentCount',
    'sourceInkComponentPixelCount',
    'outputInkComponentPixelCount',
    ...
    'shadowCoverageDrift',
    'tileShadowCoverageDriftMax',
    'tileShadowCoverageDriftP95',
    'tileShadowCoverageTileCount',
    'alphaMae',
    'alphaP95',
    'alphaMax',
    'alphaTileMaxMae',
    ...
    'saturatedCoverageDrift',
    'tileSaturatedCoverageDriftMax',
    'tileSaturatedCoverageDriftP95',
    'tileSaturatedCoverageTileCount',
    'colorfulRetention',
