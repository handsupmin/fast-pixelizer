...
function toItem({ dataset, expected, input, metrics, name, original, resized, uniformity }) {
  ...
  return {
    ...
    tileLumaMeanDeltaP95: formatNum(metrics.tileLumaMeanDeltaP95),
    tileLumaMeanDeltaTileCount: metrics.tileLumaMeanDeltaTileCount,
    tileShadowCoverageDriftMax: formatNum(metrics.tileShadowCoverageDriftMax),
    tileShadowCoverageDriftP95: formatNum(metrics.tileShadowCoverageDriftP95),
    tileShadowCoverageTileCount: metrics.tileShadowCoverageTileCount,
    tileBrightCoverageDriftMax: formatNum(metrics.tileBrightCoverageDriftMax),
    tileBrightCoverageDriftP95: formatNum(metrics.tileBrightCoverageDriftP95),
    tileBrightCoverageTileCount: metrics.tileBrightCoverageTileCount,
    ...
    sourceAlphaEdgeTileCount: metrics.sourceAlphaEdgeTileCount,
    outputAlphaEdgeTileCount: metrics.outputAlphaEdgeTileCount,
    alphaEdgeTileJaccardMin: formatNum(metrics.alphaEdgeTileJaccardMin),
    alphaEdgeTileRecallMin: formatNum(metrics.alphaEdgeTileRecallMin),
    alphaEdgeTileSpuriousMax: formatNum(metrics.alphaEdgeTileSpuriousMax),
    ...
    tileChromaRatioMin: formatNum(metrics.tileChromaRatioMin),
    tileChromaRatioMax: formatNum(metrics.tileChromaRatioMax),
    tileChromaTileCount: metrics.tileChromaTileCount,
    sourceColorfulPixelCount: metrics.sourceColorfulPixelCount,
    outputColorfulPixelCount: metrics.outputColorfulPixelCount,
    retainedColorfulPixelCount: metrics.retainedColorfulPixelCount,
    spuriousColorfulPixelCount: metrics.spuriousColorfulPixelCount,
    tileColorfulCoverageDriftMax: formatNum(metrics.tileColorfulCoverageDriftMax),
    tileColorfulCoverageDriftP95: formatNum(metrics.tileColorfulCoverageDriftP95),
    tileColorfulCoverageTileCount: metrics.tileColorfulCoverageTileCount,
    sourceBrightPixelCount: metrics.sourceBrightPixelCount,
    outputBrightPixelCount: metrics.outputBrightPixelCount,
    inputBrightCoverage: formatNum(metrics.inputBrightCoverage),
    outputBrightCoverage: formatNum(metrics.outputBrightCoverage),
    brightCoverageDrift: formatNum(metrics.brightCoverageDrift),
    sourceSaturatedPixelCount: metrics.sourceSaturatedPixelCount,
    outputSaturatedPixelCount: metrics.outputSaturatedPixelCount,
    inputSaturatedCoverage: formatNum(metrics.inputSaturatedCoverage),
    outputSaturatedCoverage: formatNum(metrics.outputSaturatedCoverage),
    saturatedCoverageDrift: formatNum(metrics.saturatedCoverageDrift),
    tileSaturatedCoverageDriftMax: formatNum(metrics.tileSaturatedCoverageDriftMax),
    tileSaturatedCoverageDriftP95: formatNum(metrics.tileSaturatedCoverageDriftP95),
    tileSaturatedCoverageTileCount: metrics.tileSaturatedCoverageTileCount,
    colorfulRetention: formatNum(metrics.colorfulRetention),
    colorfulSpuriousRatio: formatNum(metrics.colorfulSpuriousRatio),
    ...
    sourceDominantBucketCoverage: formatNum(metrics.sourceDominantBucketCoverage),
    outputDominantBucketCoverage: formatNum(metrics.outputDominantBucketCoverage),
    dominantBucketCoverageDrift: formatNum(metrics.dominantBucketCoverageDrift),
    tileRgbHistogramDriftMax: formatNum(metrics.tileRgbHistogramDriftMax),
    tileRgbHistogramDriftP95: formatNum(metrics.tileRgbHistogramDriftP95),
    tileRgbHistogramTileCount: metrics.tileRgbHistogramTileCount,
    ...
    tileHueErrorMeanMax: formatNum(metrics.tileHueErrorMeanMax),
    tileHueErrorP95Max: formatNum(metrics.tileHueErrorP95Max),
    tileHueErrorTileCount: metrics.tileHueErrorTileCount,
    contrastRatio: formatNum(metrics.contrastRatio),
    tileContrastRatioMin: formatNum(metrics.tileContrastRatioMin),
    tileContrastRatioMax: formatNum(metrics.tileContrastRatioMax),
    tileContrastTileCount: metrics.tileContrastTileCount,
    lineEdgeRatio: formatNum(metrics.lineEdgeRatio),
    tileLineEdgeRatioMin: formatNum(metrics.tileLineEdgeRatioMin),
    tileLineEdgeRatioMax: formatNum(metrics.tileLineEdgeRatioMax),
    tileLineEdgeTileCount: metrics.tileLineEdgeTileCount,
    edgeMagnitudeHistogramDrift: formatNum(metrics.edgeMagnitudeHistogramDrift),
    tileEdgeMagnitudeHistogramDriftMax: formatNum(metrics.tileEdgeMagnitudeHistogramDriftMax),
    tileEdgeMagnitudeHistogramDriftP95: formatNum(metrics.tileEdgeMagnitudeHistogramDriftP95),
    tileEdgeMagnitudeHistogramTileCount: metrics.tileEdgeMagnitudeHistogramTileCount,
    sourceDirectedEdgeBinCount: metrics.sourceDirectedEdgeBinCount,
    outputDirectedEdgeBinCount: metrics.outputDirectedEdgeBinCount,
    directedEdgeJaccardMin: formatNum(metrics.directedEdgeJaccardMin),
    directedEdgeRecallMin: formatNum(metrics.directedEdgeRecallMin),
    directedEdgeSpuriousMax: formatNum(metrics.directedEdgeSpuriousMax),
    ...
    edgeTileJaccardMin: formatNum(metrics.edgeTileJaccardMin),
...
async function buildMetrics(input, expected, snapshots, colorVariety) {
  ...
  return {
    metrics: {
      ...
      tileLumaMeanDeltaP95: preserve.tileLumaMeanDeltaP95,
      tileLumaMeanDeltaTileCount: preserve.tileLumaMeanDeltaTileCount,
      tileShadowCoverageDriftMax: preserve.tileShadowCoverageDriftMax,
      tileShadowCoverageDriftP95: preserve.tileShadowCoverageDriftP95,
      tileShadowCoverageTileCount: preserve.tileShadowCoverageTileCount,
      tileBrightCoverageDriftMax: preserve.tileBrightCoverageDriftMax,
      tileBrightCoverageDriftP95: preserve.tileBrightCoverageDriftP95,
      tileBrightCoverageTileCount: preserve.tileBrightCoverageTileCount,
      ...
      sourceAlphaEdgeTileCount: preserve.sourceAlphaEdgeTileCount,
      outputAlphaEdgeTileCount: preserve.outputAlphaEdgeTileCount,
      alphaEdgeTileJaccardMin: preserve.alphaEdgeTileJaccardMin,
      alphaEdgeTileRecallMin: preserve.alphaEdgeTileRecallMin,
      alphaEdgeTileSpuriousMax: preserve.alphaEdgeTileSpuriousMax,
      inputChromaMean: preserve.inputChromaMean,
      outputChromaMean: preserve.outputChromaMean,
      chromaRatio: preserve.chromaRatio,
      tileChromaRatioMin: preserve.tileChromaRatioMin,
      tileChromaRatioMax: preserve.tileChromaRatioMax,
      tileChromaTileCount: preserve.tileChromaTileCount,
      sourceColorfulPixelCount: preserve.sourceColorfulPixelCount,
      outputColorfulPixelCount: preserve.outputColorfulPixelCount,
      retainedColorfulPixelCount: preserve.retainedColorfulPixelCount,
      spuriousColorfulPixelCount: preserve.spuriousColorfulPixelCount,
      tileColorfulCoverageDriftMax: preserve.tileColorfulCoverageDriftMax,
      tileColorfulCoverageDriftP95: preserve.tileColorfulCoverageDriftP95,
      tileColorfulCoverageTileCount: preserve.tileColorfulCoverageTileCount,
      sourceBrightPixelCount: preserve.sourceBrightPixelCount,
      outputBrightPixelCount: preserve.outputBrightPixelCount,
      inputBrightCoverage: preserve.inputBrightCoverage,
      outputBrightCoverage: preserve.outputBrightCoverage,
      brightCoverageDrift: preserve.brightCoverageDrift,
      sourceSaturatedPixelCount: preserve.sourceSaturatedPixelCount,
      outputSaturatedPixelCount: preserve.outputSaturatedPixelCount,
      inputSaturatedCoverage: preserve.inputSaturatedCoverage,
      outputSaturatedCoverage: preserve.outputSaturatedCoverage,
      saturatedCoverageDrift: preserve.saturatedCoverageDrift,
      tileSaturatedCoverageDriftMax: preserve.tileSaturatedCoverageDriftMax,
      tileSaturatedCoverageDriftP95: preserve.tileSaturatedCoverageDriftP95,
      tileSaturatedCoverageTileCount: preserve.tileSaturatedCoverageTileCount,
      colorfulRetention: preserve.colorfulRetention,
      colorfulSpuriousRatio: preserve.colorfulSpuriousRatio,
      ...
      sourceDominantBucketCoverage: preserve.sourceDominantBucketCoverage,
      outputDominantBucketCoverage: preserve.outputDominantBucketCoverage,
      dominantBucketCoverageDrift: preserve.dominantBucketCoverageDrift,
      tileRgbHistogramDriftMax: preserve.tileRgbHistogramDriftMax,
      tileRgbHistogramDriftP95: preserve.tileRgbHistogramDriftP95,
      tileRgbHistogramTileCount: preserve.tileRgbHistogramTileCount,
      hueErrorMean: preserve.hueErrorMean,
      hueErrorP95: preserve.hueErrorP95,
      hueSampleCount: preserve.hueSampleCount,
      tileHueErrorMeanMax: preserve.tileHueErrorMeanMax,
      tileHueErrorP95Max: preserve.tileHueErrorP95Max,
      tileHueErrorTileCount: preserve.tileHueErrorTileCount,
      contrastRatio: preserve.contrastRatio,
      tileContrastRatioMin: preserve.tileContrastRatioMin,
      tileContrastRatioMax: preserve.tileContrastRatioMax,
      tileContrastTileCount: preserve.tileContrastTileCount,
      lineEdgeRatio: preserve.lineEdgeRatio,
      tileLineEdgeRatioMin: preserve.tileLineEdgeRatioMin,
      tileLineEdgeRatioMax: preserve.tileLineEdgeRatioMax,
      tileLineEdgeTileCount: preserve.tileLineEdgeTileCount,
      edgeMagnitudeHistogramDrift: preserve.edgeMagnitudeHistogramDrift,
      tileEdgeMagnitudeHistogramDriftMax: preserve.tileEdgeMagnitudeHistogramDriftMax,
      tileEdgeMagnitudeHistogramDriftP95: preserve.tileEdgeMagnitudeHistogramDriftP95,
      tileEdgeMagnitudeHistogramTileCount: preserve.tileEdgeMagnitudeHistogramTileCount,
      sourceDirectedEdgeBinCount: preserve.sourceDirectedEdgeBinCount,
      outputDirectedEdgeBinCount: preserve.outputDirectedEdgeBinCount,
      directedEdgeJaccardMin: preserve.directedEdgeJaccardMin,
      directedEdgeRecallMin: preserve.directedEdgeRecallMin,
      directedEdgeSpuriousMax: preserve.directedEdgeSpuriousMax,
      ...
      edgeTileJaccardMin: preserve.edgeTileJaccardMin,
